...
class OptimizeImageView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            img2 = self.load_image(image2) if image2 else None

        except Exception as e:
            return Response({'error': str(e)}, status=400)
        
        quality = self.get_quality(serializer)
        format_choice = serializer.validated_data.get('format_choice')

        if params.get('grayscale', False):
            img = self.convert_to_grayscale(img)

        if params.get('panorama', False) and img2 is not None:
            img = self.process_panorama(img, img2)


        encoded_img = self.encode_image(img, format_choice, quality)
        if not encoded_img:
            return Response({'error': 'The image could not be encoded.'}, status=400)
        
        image_url = self.save_image(encoded_img, format_choice)
    ...
    def load_image(self, image):
        """بارگذاری تصویر از داده‌های ورودی"""
        file_data = image.read()
        nparr = np.frombuffer(file_data, np.uint8)
        img = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
        if img is None:
    ...
    def get_quality(self, serializer):
        """دریافت کیفیت تصویر"""
        quality = serializer.validated_data.get('quality', 95)
        try:
            quality = int(quality)
            ...
            raise ValueError('Quality must be a valid integer between 1 and 100.')
        return quality

    def convert_to_grayscale(self, img):
        """تبدیل تصویر به خاکی-سفید"""
        return cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

    def process_panorama(self, img, img2):
        """ایجاد پانوراما از دو تصویر"""
    ...
    def encode_image(self, img, format_choice, quality):
        """انکود کردن تصویر در فرمت دلخواه"""
        if format_choice == 'jpeg':
            encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), quality]
            result, img_encoded = cv2.imencode('.jpg', img, encode_param)
        elif format_choice == 'png':
            ...
            return None
        return img_encoded if result else None

    def save_image(self, img_encoded, format_choice):
        """ذخیره‌سازی تصویر در سرور"""
        media_path = settings.MEDIA_ROOT
